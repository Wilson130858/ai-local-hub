...
export function BillingOverview() {
  ...
  const aggregates = useMemo(() => {
    return tenants.map((t) => {
      const tenantQuotes = quotes.filter((q) => q.tenant_id === t.id);
                ...
                <TableBody>
                  {filtered.map((row) => (
                    <TableRow key={row.tenant.id}>
                      <TableCell>
                        <div className="font-medium">{row.tenant.business_name}</div>
                        <div className="text-xs text-muted-foreground">{row.profile?.full_name ?? "—"}</div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          <Button size="sm" variant="ghost" onClick={() => openQuote(row.tenant)} title="Adicionar serviço">
                            <Plus className="h-4 w-4" />
                          </Button>
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => row.profile && openDetail(row.profile)}
              ...
              <Table>
                ...
                <TableBody>
                  {pendingGlobal.map((q) => {
                    ...
                    const profile = tenant ? profiles[tenant.owner_id] : undefined;
                    return (
                      <TableRow key={q.id}>
                        <TableCell>
